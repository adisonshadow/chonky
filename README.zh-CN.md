...
Chonky 目前处于 **第一阶段**：基于 npm 包（Babel / SWC 插件）提供完整的语法扩展与元数据能力，可立即投入生产使用。  
未来第二阶段将使用 Rust 重构编译器核心，实现 10 倍以上的编译速度提升，并开启更激进的自主性能调优（自动 WebAssembly 拆分等）。

查看完整路线图请访问 [ROADMAP.md](./docs/ROADMAP.zh-CN.md)。

---

好的，这是您可以追加在 `README.md` 末尾的贡献者欢迎说明段落：
