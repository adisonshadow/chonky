...
| **ChonkyRenderer** | `@chonky/runtime` 提供的核心类，负责元数据的收集、存储与查询 |
...
`@chonky/babel-plugin` 在编译阶段对所有 JSX 元素执行以下转换：
...
| **生产环境** (`NODE_ENV=production`) | Babel 插件不注入包裹代码；`@chonky/runtime` 中的收集逻辑被 Tree-Shaking 移除 |
...
`ChonkyRenderer` 是 `@chonky/runtime` 导出的单例类，提供以下公开方法：
