...
| 断言体内的 `expect(...)` | 保留原样（来自 `@chonky/runtime/test`） |
...
所有测试辅助函数均从 `@chonky/runtime/test` 导入：

```typescript
import { test, expect, describe, beforeEach, afterEach } from '@chonky/runtime/test';
```

`@chonky/runtime/test` 在底层委托给项目实际安装的测试框架（Jest 或 Vitest），通过以下检测顺序自动适配：
...
```typescript
import { verify } from '@chonky/runtime';
